...
[^4]: Research compendia are conceptually quite similar to **Reproducible Analytical Pipelines (RAPs)**, although the latter focuses more on production processes. Hence if the research is easy to reproduce by adopting a compendium structure and making everything easily reproducible, the operationalization of a new method could be dramatically simplified. For more on RAPs in price statistics, see a [RAP class recently done by ESCAP](https://escap-sd.github.io/ESCAP_RAP_class/docs/), as well as a good paper by [Price and Marques (2023)](https://unece.org/sites/default/files/2023-05/7.4%20UK_un_systems_railfares_paper.pdf) showcasing RAP for production processes.

## Key components in a research project

To make a research project reproducible, it is key to  observe that there is not one object that is produced (i.e., the paper), but four different objects. @fig-research-objects provides a visual that can help understand what each is intended for, and how to approach each for reproducibility.
...
-   **Input data** is the data used to answer research questions. It may be open, closed (proprietary or sensitive), or synthetic. Each is approached separately. Some notable aspects:
    ...
    -   While input data is ingested and processed by the research, in itself it is *ignored* by the research compendium (such as by listing the data files in the `.gitignore`). Read more in the [input data guide](/docs/reproducibility-guidance/howtos/input-data.qmd).

-   **Output data** is data created as part of the research process. Whether (and how) it is shared depends on various circumstances and value to later research projects. For example, it may be used as an input dataset for other projects. Read [about this process here](/docs/reproducibility-guidance/howtos/opening-data.qmd).
...
-   A snapshot of the **computational environment** should be taken and included in the research compendium. This ensures that the exact computation requirement can be replicated, but also ensures that dependencies on specific versions of other packages is saved.
